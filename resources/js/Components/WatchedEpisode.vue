...
const show = computed(() => episode.show[0]);

const date = (pivot) => {
  return dayjs.utc(pivot.created_at).fromNow()
}
</script>
  ...
  <div class="relative bg-white dark:bg-gray-950 overflow-hidden shadow-xl sm:rounded-lg py-4 flex flex-col justify-between">
    ...
    <div class="flex flex-wrap">
      <div class="mx-4 flex-grow">
        <div class="text-xs uppercase text-gray-300 tracking-widest">{{ date(episode.pivot) }}</div>
      </div>
